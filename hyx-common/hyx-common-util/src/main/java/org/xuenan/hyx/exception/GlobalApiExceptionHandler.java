...
import org.xuenan.hyx.Context;
import org.xuenan.hyx.HttpOutJson;
import org.xuenan.hyx.JSONUtils;
import org.xuenan.hyx.ListUtils;
 ...
 * @version 1.0.0
 * @Description 全局异常处理
 * @date 2019年09月02日 16:23:00
 */
@ControllerAdvice
public class GlobalApiExceptionHandler {
    private final static Logger logger = LoggerFactory.getLogger("hyx.log");
    ...
    public ModelAndView handleMethodArgumentNotValid(MethodArgumentNotValidException ex, HttpServletRequest request,
        ...
        ResponseEntity<?> responseEntity = ResponseEntity
                .fail(new GlobalException(GlobalExceptionStatus.REQUEST_INVALID_PARAMETER));
        responseEntity.setMessage(ex.getBindingResult().getFieldError().getDefaultMessage());
        responseEntity.setDetails(geJson(ex.getBindingResult()));
    ...
    public ModelAndView handleBindException(MissingServletRequestParameterException ex, HttpServletRequest request,
                                            HttpServletResponse response) {
        ResponseEntity<?> responseEntity = ResponseEntity
                .fail(new GlobalException(GlobalExceptionStatus.REQUEST_INVALID_PARAMETER));
    ...
    private static void logLevelOut(ResponseEntity<?> responseEntity, Throwable ex) {
        ListUtils<Object> lu = ListUtils.n()
                .a(responseEntity.getLevel())
                .a(responseEntity.getCode())
